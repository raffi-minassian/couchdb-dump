...
exports.couchdbdump = function() {
  ...
  if(conf.allDatabases) {
    const q = async.queue(function(task, cb) {
      dump(task.conf, task.db, task.output, 0, () => {
        output.close();
        cb();
      });
    }, conf.concurrency);
  ...
  requestPromise
    ...
    .then(function(data) {
      ...
      if(docs.rows.length > 0) {
        let dataArray = [];
        docs.rows.map((row) => {
          dataArray.push(JSON.stringify(row));
        });
        output.write(dataArray.join(','));
        dataArray = [];

        if(docs.rows.length == options.numberPerPage) {
          return dump(options, database, output, (offset+options.numberPerPage), cb)
        }
      }
      output.write(']}\n');
      cb();
